...
class Match:
        ...
        while self.running.is_set() and not stop_searching.is_set():
            try:
                ...
                # Respond to join messages with key and settings
                    ...
                    self.sock.sendto(json.dumps(match_info).encode(), addr)

                # Starts game on player join
                elif message == JOIN_NOTICE and addr == join_connection:
                    print("Player joined!")
                    self.running.clear()
    ...
    def connect(self, host_port):
        ...
        try:
            ...
            sock.sendto(JOIN_REQUEST.encode(), ("127.0.0.1", host_port))
            ...
            sock.sendto(JOIN_NOTICE.encode(), addr)
...
class MatchSearch:
    ...
    def discover_matches(self):
        ...
        for sock, port in sockets:
            try:
                sock.sendto(DISCOVERY_MESSAGE.encode(), ("127.0.0.1", port))
